...
class UploadController extends Controller {
    public function store(Request $request, Project $project, ImportSession $session) {
        ...
        if ($session->state !== 'CREATED') { abort(400, 'Session is not in CREATED state'); }
        ...
        // A2. Source Immutability: Use a temp prefix for the upload to prevent overwriting accepted artifacts
        ...
        // A1. Use temporaryUploadUrl for PUT requests
    ...
    public function complete(Request $request, Project $project, ImportSession $session, UploadAttempt $attempt) {
        ...
        // A5. Upload recovery: Handle stale VERIFYING attempts
        if ($attempt->status === 'VERIFYING' && $attempt->updated_at->diffInMinutes(now()) > 5) {
            $attempt->update(['status' => 'PENDING']); // Recovery
        }
        ...
        if (!$disk->exists($attempt->storage_key)) {
            $attempt->update(['status' => 'PENDING']); // Revert so client can retry
        ...
        if (str_ends_with($filename, '.xlsx')) {
            ...
            file_put_contents($tempPath, $disk->get($attempt->storage_key));
            $zip = new \ZipArchive();
            if ($zip->open($tempPath) === true) {
                if ($zip->locateName('[Content_Types].xml') === false && $zip->locateName('xl/workbook.xml') === false) {
                    ...
                    unlink($tempPath);
                    $attempt->update(['status' => 'REJECTED']);
                    return response()->json(['message' => 'Malformed XLSX file: Missing structural components.'], 400);
                }
                $zip->close();
            } else {
                unlink($tempPath);
                $attempt->update(['status' => 'REJECTED']);
                return response()->json(['message' => 'Malformed ZIP/XLSX file.'], 400);
            }
            unlink($tempPath);
        ...
        // A2. Source Immutability: Move to immutable storage key
        ...
        return DB::transaction(function() use ($request, $project, $session, $attempt, $size, $type, $finalKey, $disk) {
            ...
            // Phase 3: Dispatch job to Python Data Engine
            try {
                \Illuminate\Support\Facades\Http::post('http://127.0.0.1:8001/api/v1/jobs/inspect', [
                    'session_id' => $session->id,
                    'storage_key' => $artifact->storage_key,
                    'content_type' => $artifact->content_type,
                    'callback_url' => url('/api/v1/internal/jobs/callback'),
                    'internal_token' => config('app.internal_token', 'secret-token')
                ]);
            } catch (\Exception $e) {
                Log::error("Failed to dispatch inspection job: " . $e->getMessage());
            }
            
            // Delete temp file after transaction is safely committed
